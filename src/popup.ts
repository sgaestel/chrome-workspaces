(function() {
  ...
  const workspacesDiv = <HTMLElement>document.getElementById('workspaces');
  const currentWorkspaceDiv = <HTMLElement>document.getElementById('currentWorkspace');
  ...
  const displayWorkspaceLine = (currentWorkspace: string, name: string, numberOfTabs: number, line: number) => {
    const onClick = () => {
  ...
  const workspacesLoaded = (res: StorageResponse) => {
